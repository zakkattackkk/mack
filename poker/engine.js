...
(function (root) {
  ...
  // Classifies two hole cards into a tier that drives preflop advice.
  // Returns { tier: 'premium'|'strong'|'playable'|'trash', label: 'AKs' }.
  // Raw showdown equity understates premiums in a multiway pot (you fold
  // later when you miss, and you have fold equity), so preflop we lean on
  // hand strength, not just pot odds — which is why AK is a raise, not a fold.
  function preflopTier(hole) {
    var r1 = cardRank(hole[0]), r2 = cardRank(hole[1]);
    var hi = Math.max(r1, r2), lo = Math.min(r1, r2);
    var suited = cardSuit(hole[0]) === cardSuit(hole[1]);
    var pair = hi === lo;
    var gap = hi - lo;
    var label = RANKS[hi] + RANKS[lo] + (pair ? '' : (suited ? 's' : 'o'));
    // rank codes: A=12 K=11 Q=10 J=9 T=8 9=7 ...
    var tier;
    if (pair && hi >= 9) tier = 'premium';                 // JJ, QQ, KK, AA
    else if (hi === 12 && lo === 11) tier = 'premium';     // AK (s or o)
    else if (pair && hi >= 7) tier = 'strong';             // 99, TT
    else if (hi === 12 && lo === 10) tier = 'strong';      // AQ
    else if (hi === 12 && lo >= 8 && suited) tier = 'strong'; // AJs, ATs
    else if (hi === 11 && lo === 10 && suited) tier = 'strong'; // KQs
    else if (pair) tier = 'playable';                      // 22-88
    else if (hi === 12 && suited) tier = 'playable';       // any suited ace
    else if (hi === 12 && lo >= 8) tier = 'playable';      // AJo, ATo
    else if (hi >= 10 && lo >= 8) tier = 'playable';       // KQ/KJ/QJ/QT/KT broadways
    else if (suited && gap <= 1 && lo >= 3) tier = 'playable'; // suited connectors 54s+
    else if (suited && hi === 11 && lo >= 6) tier = 'playable'; // Kxs decent
    else tier = 'trash';
  ...
  function advise(params) {
    ...
    // Is this a preflop decision? (hole given, no community cards yet)
    var hole = params.hole;
    var board = params.board || [];
    var preflop = hole && hole.length === 2 && board.length === 0;
    ...
    if (preflop) {
      ...
      if (toCall === 0) {
        ...
        if (pf.tier === 'premium') {
          ...
          action = 'FOLD';
          reason = pf.label + ' isn\'t worth calling a raise. Save your chips for a better spot.';
        }
      }
      return {
        requiredEquity: required, evCall: evCall, action: action, reason: reason,
        suggestBet: suggestBet, betText: betText, tier: pf.tier, handLabel: pf.label
      };
    }
